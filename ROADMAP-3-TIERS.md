 ...
 **Goal:** Make the first-run experience smoother with minimal scope changes.
 ...
 ### Scope
 1. **Output hydration prompt in terminal**
    - CLI prints the hydration prompt directly after install in a clear,
      copyable block.
    - Keep file output as a fallback, but do not require the user to find it.
    - Add TTY detection and a quiet mode for CI logs.
 ...
 ### Deliverables
 - CLI prints prompt in terminal with a labeled copy block.
 ...
 ### Acceptance Criteria
 - New install flow can be completed without opening any prompt file.
 ...
 **Goal:** Offer a "lighter" entry point and reduce structural assumptions.
 ...
 ### Scope
 1. **Zero-config mode**
    - Install only `.cursor/rules` and minimal docs.
